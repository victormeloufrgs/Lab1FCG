...
int totalVertices = 32;
int indicesLength = totalVertices+2; // +2 pois os dois últimos são pra fechar os triângulos.
...
std::vector<Vertex> getDonutVertexArray(int numberOfVertex, float raioExterno, float raioInterno);
...
GLuint BuildTriangles()
{
    ...
    std::vector<Vertex> vertexArray = getDonutVertexArray(totalVertices, 0.7, 0.5);
...
std::vector<Vertex> getDonutVertexArray(int numberOfVertex, float raioExterno, float raioInterno) {
    ...
    for(double currentAngle = 0; currentAngle < 360.0; currentAngle+=angleIncrement) {
        if (i % 2 == 0) {
            ...
            circleVertexArray.push_back(getVertex(origin, currentAngle, raioInterno));
        }
        i++;
    }
